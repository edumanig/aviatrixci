...
class S2C_Tests(unittest.TestCase):
    ...
    def test_s2c(self):
        ...
        ssh_username = self.cases.case_data['username']
        ssh_password = self.cases.case_data['password']
        ...
        for case in self.case_list:
            if "test_case" in case:
                ...
                ssh_client = launch_ssh_client(self.logger,VPC1_VM_PUB_IP, ssh_username, ssh_password)

                self.logger.info("Ping the target IP %s", VPC2_VM_PRV_IP)
                if self.cases.case_data['03.conn_type'].lower() == "unmapped":
                    self.assertTrue(ping_aws_user_instance(self.logger, ssh_client, VPC2_VM_PRV_IP,cloud_region))
                if self.cases.case_data['03.conn_type'].lower() == "mapped":
                    self.assertTrue(ping_aws_user_instance(self.logger, ssh_client, VPC2_VM_VIRT_PRV_IP,cloud_region))
