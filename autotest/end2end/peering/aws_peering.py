...
from autotest.lib.vnet_utils import ping_between_vm
from autotest.frontend.webuitest import *
from autotest.lib.test_utils import testcases

class AWSENCPeering(WebUITest):
    cases = testcases(__name__)

    def test01_encrypted_peering(self):
        ...
        for encpeering in self.cases.case_data:
            self.logger.info("Delete the ENC peering")
            self.assertTrue(peering_view.click_delete_enc_peering_button(self.cases.case_data[encpeering]["gateway1"]),
                            "Failed to click the button for deleting the ENC peering over AWS peering")
            ...
            self.assertIn(self.cases.expected_result["toaster_delete"], result, "Failed to delete the ENC peering")

            time.sleep(10)
            self.logger.info("Verifying deleted peering is no longer in ENC peering table")
            self.assertFalse(
                peering_view.enc_peering_table.is_data_present(self.driver, 1, self.cases.case_data[encpeering]["gateway1"]),
            ...
            peering_view.select_vpc1 = self.cases.case_data[encpeering]['gateway1']
            time.sleep(3)
            self.logger.info("Select Gateway 2 for ENC peering")
            peering_view.select_vpc2 = self.cases.case_data[encpeering]['gateway2']
            time.sleep(3)
            ...
            self.assertEqual(
                peering_view.enc_peering_table.check_specific_row_data(self.driver, self.cases.case_data[encpeering]['gateway1'], 1, 0),
                self.cases.case_data[encpeering]['gateway2'], "Failed to find the ENC peering in the table")

            time.sleep(10)
            self.logger.info('Start to test the encrypted peering tunnel; Ping both ways')
            self.assertTrue(ping_between_vm(self.logger,self.cases.case_data[encpeering]["vpc_name_2"],self.cases.case_data[encpeering]["user_vm_public_1"],self.cases.case_data[encpeering]["username"],self.cases.case_data[encpeering]["password"],self.cases.case_data[encpeering]["user_vm_private_2"]))
            self.assertTrue(ping_between_vm(self.logger, self.cases.case_data[encpeering]["vpc_name_1"],
                                                   self.cases.case_data[encpeering]["user_vm_public_2"],
                                                   self.cases.case_data[encpeering]["username"],
                                                   self.cases.case_data[encpeering]["password"],
                                                   self.cases.case_data[encpeering]["user_vm_private_1"]))
    ...
    def test02_transitive_peering(self):
        ...
        self.logger.info("Start to create the transitive peering")
        ...
        self.assertEqual(
            peering_view.transitive_peering_table.check_specific_row_data(self.driver, self.cases.case_data['source'],
                                                                          1),
            self.cases.case_data['nexthop'], "Failed to find the Transitive peering in the table")

        self.logger.info('Start to test the transitive peering tunnel; Ping from Source to Destination')
        self.assertTrue(ping_between_vm(self.logger, self.cases.case_data["vpc_name"],
                                        self.cases.case_data["user_vm_public_src"],
                                        self.cases.case_data["username"],
                                        self.cases.case_data["password"],
                                        self.cases.case_data["user_vm_private_dest"]))


        self.cases.end_test("test_case_2")

    def test03_over_aws_peering(self):
        peering_view = prng.Peering(self.driver)
        actions_in_common = actions.ActionsInCommon(self.driver)
        ...
        self.assertIn(self.cases.expected_result['toaster'], result,
                      "Failed to create the ENC peering over AWS Peering")
        actions_in_common.close_message()

        time.sleep(20)
        self.logger.info("Checking new ENC peering is found in ENC peering table")
        self.assertEqual(
            peering_view.enc_peering_table.check_specific_row_data(self.driver, self.cases.case_data["gateway1"], 8),
            "yes", "Failed to find the ENC peering in the table")
        time.sleep(10)
        self.logger.info('Start to test the encrypted peering tunnel; Ping both ways')
        self.assertTrue(ping_between_vm(self.logger, self.cases.case_data["vpc_name_2"],
                                        self.cases.case_data["user_vm_public_1"],
                                        self.cases.case_data["username"],
                                        self.cases.case_data["password"],
                                        self.cases.case_data["user_vm_private_2"]))
        self.assertTrue(ping_between_vm(self.logger, self.cases.case_data["vpc_name_1"],
                                        self.cases.case_data["user_vm_public_2"],
                                        self.cases.case_data["username"],
                                        self.cases.case_data["password"],
                                        self.cases.case_data["user_vm_private_1"]))

        self.logger.info("Delete the ENC peering over AWS peering")
        ...
        time.sleep(10)
        self.logger.info("Verify the deleted peering is no longer found in ENC peering table")
        self.assertFalse(
            peering_view.enc_peering_table.is_data_present(self.driver, 1, self.cases.case_data["gateway1"]),
            "Unexpectedly found the specified ENC peering")
        actions_in_common.close_message()
