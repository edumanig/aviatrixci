...
class ENCPeering(WebUITest):
    ...
    def test01_encrypted_peering(self):
        ...
        for encpeering in self.cases.case_data:
            ...
            self.assertTrue(ping_between_vm(self.logger,self.cases.case_data[encpeering]["vnet_name_2"],self.cases.case_data[encpeering]["user_vm_public_1"],self.cases.case_data[encpeering]["username"],self.cases.case_data[encpeering]["password"],self.cases.case_data[encpeering]["user_vm_private_2"]))
            self.assertTrue(ping_between_vm(self.logger, self.cases.case_data[encpeering]["vnet_name_1"],
    ...
    def test02_transitive_peering(self):
        ...
        self.assertTrue(ping_between_vm(self.logger, self.cases.case_data["vnet_name"],
    ...
    def test03_delete_enc_peering(self):
        ...
        for encpeering in self.cases.case_data:
            self.assertTrue(peering_view.click_delete_enc_peering_button(self.cases.case_data[encpeering]),
                            "Failed to click the button for deleting the ENC peering")

            time.sleep(5)
            self.logger.info("Clicking OK to delete "+encpeering)
            actions_in_common.confirm_ok()

            self.logger.info("Checking the result...")
            result = actions_in_common.get_message()
            self.assertIn(self.cases.expected_result["toaster"], result, "Failed to delete "+ encpeering)

            time.sleep(10)
            self.logger.info("Verifying deleted peering is no longer in ENC peering table")
            self.assertFalse(peering_view.enc_peering_table.is_data_present(self.driver, 1, self.cases.case_data[encpeering]),
                             "Unexpectedly found the peering"+ encpeering)
            actions_in_common.close_message()
