...
class CreateENCPeering(WebUITest):
    ...
    def test03_create_enc_peering_over_aws(self):
        ...
        self.logger.info("Checking the result of creating the ENC peering...")
        result = actions_in_common.get_message()
        self.assertIn("are not in the same region... Abort!!!", result, "Unexpectedly created the ENC peering over AWS Peering")
        actions_in_common.close_message()
        ...
        self.logger.info("Checking the result of creating the ENC peering...")
        result = actions_in_common.get_message()
        self.assertIn(self.cases.expected_result['toaster'], result, "Failed to create the ENC peering over AWS Peering")
        actions_in_common.close_message()

        time.sleep(20)
        self.logger.info("Checking new ENC peering is found in ENC peering table")
        self.assertEqual(peering_view.enc_peering_table.check_specific_row_data(self.driver, self.cases.case_data["gateway1"], 7),
