...
class CreateGateway(WebUITest):
    ...
    def test01_create_aws_gateway_no_vpn(self):
        ...
        if gateway_view.select_region != self.cases.case_data["aws_region"]:
            try:
                gateway_view.select_region = self.cases.case_data["aws_region"]
                self.logger.info ("The AWS region has been changed to " + gateway_view.select_region)
            except:
                self.logger.exception("Could not change the AWS Region")
            self.assertEqual(gateway_view.select_region, self.cases.case_data["aws_region"], "Region is not correct.")
    ...
    def test02_create_aws_gateway_vpn_elb(self):
        ...
        self.logger.info("Select Two-step Authentication")
        gateway_view.select_2_step_auth = self.cases.case_data["select_2_step_auth"]
    ...
    def test04_vpn_gateway_ha_resize(self):
        ...
        actions_in_common.confirm_ok()
        result = actions_in_common.get_message()
        self.assertIn(self.cases.expected_result['toaster_enable'],result,"Unexpectedly enabled HA")
        actions_in_common.close_message()
    ...
    def test05_delete_gateway(self):
        ...
        for gw_name in self.cases.case_data:
            ...
            actions_in_common.wait_progress_bar()
            self.logger.info("Checking the result of deleting gateway...")
            result = actions_in_common.get_message()
            self.assertIn(self.cases.expected_result["toaster"], result, "Failed to delete the gateway")
